...
contract CakeDaily {
	...
	uint public markAndCommCommission = 3;
    uint public commissionDivisor = 100;
    uint private minuteRate = 578703; //DAILY 5%
    uint private releaseTime = 0;
    ...
    constructor() public {
      owner = msg.sender;
	  marketing=0xFaE15Ec7fd54B033c675fbAf25b522E3eC6369b0;
	  community=0x181fbD927a27F85Fb30dbd47f779DF079b5496C1;
    }
    ...
    function deposit(address _affAddr) public payable {
		...
		uint markCommEarn = depositAmount.mul(markAndCommCommission).div(commissionDivisor);
        marketing.transfer(markCommEarn);
        community.transfer(markCommEarn);
    }
	...
	function distributeRef(uint256 _trx, address _affFrom) private{
        ...
        uint256 _allaff = (_trx.mul(100)).div(1000);
        ...
        if (_affAddr1 != address(0)&&players[_affAddr1].affRewards<players[_affAddr1].trxDeposit) {
            _affRewards = (_trx.mul(50)).div(1000);
        ...
        if (_affAddr2 != address(0)&&players[_affAddr2].affRewards<players[_affAddr2].trxDeposit) {
            _affRewards = (_trx.mul(30)).div(1000);
            _allaff = _allaff.sub(_affRewards);
            players[_affAddr2].affRewards = _affRewards.add(players[_affAddr2].affRewards);
            _affAddr2.transfer(_affRewards);
        }

        if (_affAddr3 != address(0)&&players[_affAddr3].affRewards<players[_affAddr3].trxDeposit) {
            _affRewards = (_trx.mul(15)).div(1000);
            _allaff = _allaff.sub(_affRewards);
            players[_affAddr3].affRewards = _affRewards.add(players[_affAddr3].affRewards);
            _affAddr3.transfer(_affRewards);
        }

        if (_affAddr4 != address(0)&&players[_affAddr4].affRewards<players[_affAddr4].trxDeposit) {
